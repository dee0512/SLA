...
def train(env_name="MountainCarContinuous-v0", seed=0, p=1):
    ...
    for l in range(num_layers):
        ...
        k["state_dim"] = state_dim - len(layers['layer'+str(l+1)])
        if l < num_layers - 1:
        ...
        eval_actions.append([])
        get_states.append(States(layers['layer'+str(l+1)]))
    ...
    for t in range(int(max_timesteps)):
        ...
        for i, pol in enumerate(policies):
            ...
            if t < start_timesteps:
                ...
                a = (pol.select_action(get_states[i].get_state(state)) + noise()).clip(-max_action, max_action)
                if i < num_layers-1:
                    skip = pol.select_skip(get_states[i].get_state(state), a)
        ...
        for i, buffer in enumerate(buffers):
            buffer.add(get_states[i].get_state(state), layer_actions[i], get_states[i].get_state(next_state), reward - (p if ((i < num_layers-1) and layer_skips[i] > 0) else 0), done_bool)
            if i < num_layers -1:
                skip_buffers[i].add(get_states[i].get_state(state), layer_actions[i], layer_skips[i], get_states[i].get_state(next_state), layer_actions[i], reward - (p if (layer_skips[i] > 0) else 0), done_bool)
        ...
        if (t + 1) % eval_freq == 0:
            ...
            for _ in range(10):
                ...
                while not eval_done:
                    for i, pol in enumerate(policies):
                        eval_action = pol.select_action(get_states[i].get_state(eval_state))
                        if i < num_layers - 1:
                            eval_skip = np.argmax(pol.select_skip(get_states[i].get_state(eval_state), eval_action))
                            if eval_skip == 0:
            ...
            for i in range(num_layers):
                ...
                for _ in range(10):
                    ...
                    eval_episode_timesteps = 0
                    while not eval_done:
                        eval_action = policies[i].select_action(get_states[i].get_state(eval_state))
                        eval_next_state, eval_reward, eval_done, _ = eval_env.step(eval_action)
                        eval_state = eval_next_state
                        eval_episode_timesteps += 1
                        task_reward += eval_reward
